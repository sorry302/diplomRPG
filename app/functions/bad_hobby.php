<?php
session_start();
require_once __DIR__ . '/db.php';
require_once __DIR__ . '/../services/ExperienceService.php';
require_once __DIR__ . '/../services/AchievementService.php';

$userId = $_SESSION['user']['user_id'] ?? null;
if (!$userId) {
    header('Location: /index.php');
    exit;
}

$userId = (int)$userId;
$activityId = (int)($_POST['activity_id'] ?? 0);
$quantity   = max(1, (int)($_POST['quantity'] ?? 1)); // количество повторений

// Получаем активность
$result = mysqli_query($conn, "
    SELECT *
    FROM activities
    WHERE id = $activityId AND category = 'bad_habit'
");

$activity = mysqli_fetch_assoc($result);

if (!$activity) {
    header('Location: /index.php');
    exit;
}

// Расчёт эффектов
$expChange         = $activity['exp_change'] * $quantity;
$spiritualChange   = $activity['spiritual_change'] * $quantity;
$physicalChange   = $activity['physical_change'] * $quantity;
$healthChange   = $activity['health_change'] * $quantity;

mysqli_begin_transaction($conn);

try {

    // Лог активности
    mysqli_query($conn, "
        INSERT INTO activity_logs (user_id, activity_id, quantity)
        VALUES ($userId, $activityId, $quantity)
    ");

    // Духовность
    mysqli_query($conn, "
        UPDATE user_stats
        SET value = LEAST(100, GREATEST(0, value + $spiritualChange))
        WHERE user_id = $userId AND stat_code = 'spiritual'
    ");

    // Физическая
    mysqli_query($conn, "
        UPDATE user_stats
        SET value = LEAST(100, GREATEST(0, value + $physicalChange))
        WHERE user_id = $userId AND stat_code = 'physical'
    ");

    // Интеллект
    mysqli_query($conn, "
        UPDATE user_stats
        SET value = LEAST(100, GREATEST(0, value + $healthChange))
        WHERE user_id = $userId AND stat_code = 'health'
    ");

    // Опыт
    $expService = new ExperienceService($conn);
    $expService->addExp($userId, $expChange);

    mysqli_commit($conn);

    // Достижения
    $achievement = new AchievementService($conn, $userId);
    $achievement->checkAll();

    $_SESSION['action_feedback'] = [
        ['label' => 'Духовность', 'value' => $spiritualChange],
        ['label' => 'Здоровье', 'value' => $healthChange],
        ['label' => 'Физическая', 'value' => $physicalChange],
        ['label' => 'Опыт', 'value' => $expChange],
    ];

} catch (Throwable $e) {
    mysqli_rollback($conn);
    echo "Ошибка: " . $e->getMessage();
    exit;
}

header('Location: /index.php');
exit;